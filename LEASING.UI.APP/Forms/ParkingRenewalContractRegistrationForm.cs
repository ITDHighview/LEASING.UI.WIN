...
{
    public partial class ParkingRenewalContractRegistrationForm : Form
    {
        ...
        public ParkingRenewalContractRegistrationForm()
        {
            InitializeComponent();
        ...
        private void M_GetPostDatedCountMonth()
        {
            ...
            {
                dgvpostdatedcheck.DataSource = null;
                using (DataSet dt = ComputationContext.GetPostDatedCountMonthParking(dtpStartDate.Text, dtpFinishDate.Text, this.RentalNoCommaFormat(txtRental.Text), null, M_getXMLData()))
                {
                    if (dt != null && dt.Tables.Count > 0 && dt.Tables[0].Rows.Count > 0)
                    {
        ...
        private void dgvList_CellClick(object sender, Telerik.WinControls.UI.GridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                else if (this.dgvList.Columns[e.ColumnIndex].Name == "ColRemoved")
                {
                    ...
                    if (MessageBox.Show("Are you sure you want to Decline this Contact computation?", "System Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            {
                                MessageBox.Show("Contact has been declined successfully !", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                M_GetComputationList();
                            }
                        }
                        catch (Exception ex)
        ...
        private int? SetUnitRecId()
        {
            if (this.UnitRecId > 0)
            {
                return this.UnitRecId;
            }
            else
            {
                return null;
        ...
        private void btnCheckunits_Click(object sender, EventArgs e)
        {
            if (ddlProject.SelectedIndex >= 0)
            {
                ...
                if (forms.IsProceed == true)
                {
                    ...
                    this.ddlUnitNumber.SelectedValue = this.UnitRecId;
                    if (this.UnitRecId > 0)
                    {
        ...
        {
            ClientBrowse forms = new ClientBrowse();
            forms.ShowDialog();
            if (forms.IsProceed)
            {
        ...
        {
            this.lblContractState.Text = RENEWAL_CONTRACT;
            this.lblContractState.ForeColor = RenewalContract;
            this.btnNewComputation.Text = "Renew Contract";
        }
