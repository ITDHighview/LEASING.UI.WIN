...
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[sp_GetComputationById] @RecId INT = NULL
AS
    BEGIN
        ...
        SELECT
                ...
                FORMAT(
                          CAST(ISNULL([tblUnitReference].[SecDeposit], 0)
                               + ISNULL([tblUnitReference].[WaterAndElectricityDeposit], 0) AS DECIMAL(10, 2)), 'N2'
                      )                                                                      AS [SecDeposit],
                ...
                FORMAT(ISNULL([tblUnitReference].[AdvancePaymentAmount], 0), 'N2')           AS [TwoMonAdvance],
                IIF(ISNULL([tblUnitReference].[IsFullPayment], 0) = 1,
                    ...
                    CAST((ISNULL([tblUnitReference].[AdvancePaymentAmount], 0)
                          + (ISNULL([tblUnitReference].[SecDeposit], 0)
                             + ISNULL([tblUnitReference].[WaterAndElectricityDeposit], 0)
                            )
                         )
                ...
                FORMAT(CAST(ISNULL([PAYMENT].[TotalPayAMount], 0) AS DECIMAL(18, 2)), 'N2')  AS [TotalPayAMount],
                FORMAT(ISNULL([tblUnitReference].[FirtsPaymentBalanceAmount], 0), 'N2')      AS [FirtsPaymentBalanceAmount],
