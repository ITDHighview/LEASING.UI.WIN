...
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[sp_GetQuickLedgerListInquiry]
...
AS
    BEGIN
        ...
        SELECT
                ...
                FORMAT(
                          (ISNULL([#tempMonthDecalrationFinal].[LedgRentalAmount], 0)
                ...
                CASE
                    WHEN ISNULL([#tempMonthDecalrationFinal].[IsPaid], 0) = 1
                        ...
                        THEN
                        IIF(ISNULL([tblUnitReference].[IsDeclineUnit],0) = 0, 'PAID', 'DECLINED')
                    WHEN ISNULL([#tempMonthDecalrationFinal].[IsPaid], 0) = 0
                         AND ISNULL([#tempMonthDecalrationFinal].[IsHold], 0) = 1
                        THEN
                        IIF(ISNULL([tblUnitReference].[IsDeclineUnit],0) = 0, 'HOLD', 'DECLINED')
                    WHEN [#tempMonthDecalrationFinal].[LedgMonth] IN
                         ...
                         AND ISNULL([#tempMonthDecalrationFinal].[IsHold], 0) = 0
                        THEN
                        IIF(ISNULL([tblUnitReference].[IsDeclineUnit],0) = 0, 'DUE', 'DECLINED')
                    WHEN CONVERT(VARCHAR(20), [#tempMonthDecalrationFinal].[LedgMonth], 107) = CONVERT(
                        ...
                        IIF(ISNULL([tblUnitReference].[IsDeclineUnit],0) = 0, 'DUE', 'DECLINED')
                    ELSE
                        IIF(ISNULL([tblUnitReference].[IsDeclineUnit],0) = 0, 'PENDING', 'DECLINED')
        ...
        --'0.00' [PenaltyAmount]
        FROM
                [dbo].[#tempMonthDecalrationFinal] WITH (NOLOCK)
            LEFT JOIN
                [dbo].[tblUnitReference]
                    ON [#tempMonthDecalrationFinal].[ReferenceID] = [tblUnitReference].[RecId]
        --WHERE
        --    [tblMonthLedger].[ReferenceID] = @ReferenceID
        --    AND [tblMonthLedger].[ClientID] = @ClientID
        ORDER BY
